...
目录库卡片的「正文状态」字段已回写完成 —— 15 张卡片标为「正文已获取」，
目录库副本同步重建，源卡片与库内副本已逐份核对一致。
...
这是**有意的取舍，不再改回 `quote=true`**：打开 `quote` 会让 RAGFlow 追加自己的引用规则
（要求用 `[ID:i]` 标注来源），盖过本题要求的「标准编号 + 条款号」格式（见「关键发现」第 4 条）；
而 `/api/retrieval` 给出的引用不仅更快，还带相似度与来源库，信息更全。
...
- ~~目录库卡片的「正文状态」字段已过期~~（**已修复**：15 张卡片已回写为「正文已获取」，
  目录库副本同步重建；2026-09-19 逐份核对源卡片与库内副本，字段一致。
  见 `ragflow-import/18-目录库重建记录.md`）
- **4 份已作废标准有正文但不作主推荐**（`SF/Z JD0301003-2015`、`SF/Z JD0401002-2015`、
  `SF/Z JD0402001-2014`、`SF/Z JD0403001-2014`，分别被 `SF/T 0151-2023`、`SF/T 0157-2023`、
  `SF/T 0156-2023`、`SF/T 0158-2023` 代替）。
  这 4 份的 `primary_recommendation` 保持 `false`、复核状态为「追溯导入」，
  用途是核对**历史**鉴定行为（当年那批检材该按哪版做）。
  这是**有意约束，不是缺陷**：放开会让已作废标准被当作现行标准推荐出去，
  在取证这种强合规场景属于实质性错误。替代关系已写在卡片的「替代关系」字段里，
  助手仍能说明"该版已被某现行版代替"，只是不作为首选给出。
  将来若确需覆盖历史适用场景，应**新增**一个历史推荐字段，而不是放开
